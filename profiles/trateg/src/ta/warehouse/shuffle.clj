...
(defn shuffle-bar-series [ds]
  (let [open  (-> ds :open fun/log)
        ...
        ds-log (tablecloth/add-columns ds {:open open
                                           :open-1 open-1
                                           :close close
                                           :open-chg open-chg
                                           :close-chg close-chg
                                           ...
                                           :low low
                                           :low-chg low-chg})
        ds-log-first (tablecloth/first ds-log)
        index-shuffled (shuffle (range 1 (tablecloth/row-count ds-log)))
        ds-log-rest (tablecloth/select-rows ds-log index-shuffled)
        open-log-0 (-> ds-log-first :open first)
        ds-shuffled (tablecloth/concat ds-log-first ds-log-rest)
    ...
    (-> ds-shuffled
        (tablecloth/add-columns {:open-s open-s
                                 :close-s close-s
                                 :high-s high-s
                                 :low-s low-s
                                 :open-p open-p
                                 ...
                                 :high-p high-p
                                 :low-p low-p})
        (tablecloth/drop-columns [:open :close :high :low :open-1])
        (tablecloth/drop-columns [:low-chg :open-chg :close-chg :high-chg])
        (tablecloth/drop-columns [:open-s  :close-s :high-s :low-s])
        (tablecloth/rename-columns {:open-p :open
                                    :close-p :close
                                    :high-p :high
                                    :low-p :low})
        (tablecloth/add-column :date (:date ds))
        )
    ;open-log-0
    ))
...
(comment
  (as-> (tablecloth/dataset {:date (range 11)
