...
(ns tv
  (:require
   [user :refer [println jsx->clj tv-widget-atom chart-active alert get-tradingview-datafeed  extract-period get-bars-random run-cb]]
   [r]))
...
(defn set-range [{:keys [from to] :as range} opts] ; Date.UTC (2012, 2, 3) / 1000,
  (let [chart (chart-active)
...
(defn request-shapes [algo symbol frequency options epoch-start epoch-end]
  (let [cb (fn [[_ data]] ;  _ = event-type
             (let [{:keys [result error]} data
                   batch-add (fn []
                               (doall
                                (map #(add-shapes (:points %) (:override %)) result)))]
