...
(defn tradingview-algo-feed [algo-ctx]
  ...
  (clj->js
   {:onReady (fn [onConfigCallback]
               (let [rp (clj 'ta.tradingview.handler-datasource/get-server-config)]
                 (-> rp
                   (p/then (fn [result]
    ...
    :searchSymbols (fn [userInput exchange symbolType onResultReadyCallback]
                     (let [query userInput
                           type symbolType
                           limit 10
                           rp (clj 'ta.tradingview.handler-datasource/symbol-search query type exchange limit)]
                       (-> rp 
                           (p/then (fn [result]
    ...
    :resolveSymbol (fn [symbolName onSymbolResolvedCallback onResolveErrorCallback]
                     ...
                     (let [rp (clj 'ta.tradingview.handler-datasource/symbol-info symbolName)]
                       (-> rp
                           (p/then (fn [result]
