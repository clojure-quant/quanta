...
(defn calc-chart-pane [algo-context pane-idx PineJS]
  (fn []
    (clj->js
     {:init (fn [_context _inputCallback]
      ...
      :main (fn [context _inputCallback]
              ...
              (let [;t (-> PineJS .-Std (.updatetime context)) ;var v = PineJS.Std.updatetime (this._context);
                    ...
                    ;v Double/NaN
                    time-idx 100
                    row-vals-vec (get-pane-data algo-context pane-idx time-idx)
                    ]
                 ;(println "time: " t ) 
                 (clj->js row-vals-vec)))})))
...
(defn study-chart-pane [algo-ctx PineJS pane-meta]
  (clj->js
   {:name (:name pane-meta)
    :metainfo pane-meta
...
(defn study-chart-studies [algo-ctx PineJS]
  (let [chart-spec (get-chart-spec algo-ctx)
        ...
        panes (get-panes algo-name chart-spec)]
    (map #(study-chart-pane algo-ctx PineJS %)
         panes)))
